...
server <- function(input, output) {
  ...
  output$map <- renderLeaflet({
    ...
    if(!is.null(input$map_shape_click)) {
      ...
      if(!is.null(buffer)) {
        ...
        segmInBuffer = segmentUnic %>% st_join(buffer %>% select(geometry),left = FALSE)
  ...
  observe({ 
    ...
    if(exists('segmInBuffer') & !is.null(event)) {
    ####################################################
    ### Datos serie
    bb =  pruebaSS %>% filter(ID_segmento %in% segmInBuffer$ID_segmento)
    # aa =  pruebaSS %>% filter(ID_segmento == event$id & !is.na(ordenSev))
    
    databb = grillaDiaHora %>% 
      left_join(bb %>% dplyr::select(diaStr,hora,n,N34,porcDH,porcDH_N34,ordenSev,ID_parada), by = c("diaStr","hora")) %>%
      mutate(across(c('n','N34','porcDH','porcDH_N34'),putNa0)) %>%
      left_join(datSTM_df_agg, by = c('ID_parada','hora')) %>%
      mutate(across(c('pasadas_media','pasadas_media_sqrt','pasadas_media_log'),putNa0))
    
    
    
    databbAgg = databb %>% mutate(N34_log = N34*(1 + pasadas_media_log)) %>%
      group_by(diaStr,finDeSem,diaSem,hora) %>%
      summarise(N34_log_Sum = sum(N34_log)) %>%
      ungroup() %>%
      mutate(date = ifelse(hora < 10,paste0(diaStr,' 0',hora,':00:00'),paste0(diaStr,' ',hora,':00:00')),
                           date = as.POSIXct(strptime(gsub('T',' ',date), "%Y-%m-%d %H:%M:%S")),
                           value = N34_log_Sum,
                           diaSem = factor(diaSem,levels = c('lunes','martes','miércoles','jueves','viernes','sábado','domingo')))
    
    fit = lm(value~ factor(diaSem) + factor(hora),data = databbAgg)
    databbAgg$pred = predict(fit,databbAgg)
    
    pb <- databbAgg %>% #filter(finDeSem == 'Lunes a viernes' & diaSem == 'viernes') %>%
      ggplot(aes(x=date, y=value)) +
      # geom_area(fill="#69b3a2", alpha=0.5) +
      # geom_area(fill="#69b3a2", alpha=0.5) +
      geom_line(color="#69b3a2",size = 0.1) +
      geom_point(aes(color = diaSem),size = 1) + 
      geom_line(aes(x=date, y=pred),color="red",size = .3) +
    ...
    output$plotSerieb <- renderPlotly(pb)
