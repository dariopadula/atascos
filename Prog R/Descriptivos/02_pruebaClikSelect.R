...
ui <- fluidPage(
  ...
  verticalLayout(
    ...
    # the map itself
    mainPanel(
      box(checkboxInput('buffer','Buffer',value = FALSE),
          numericInput('tamBuffer','Tamano buffer metros',value = 0,min = 0, max = 2000)),
      leafletOutput("map",width="100%",height="500px"),#,height="700px"
...
server <- function(input, output) {
  ...
  output$map <- renderLeaflet({
    ...
    pp = leaflet() %>% # ABRE LA VENTANA PARA HACER EL MAPA
      ...
      addLegend(position = 'topleft',pal = pal,values = segmSelect$N34medio_All,
                title = 'Total apariciones',group = 'Congestion')
  ...
  observe({ 
    ...
    message <- message %>% select(-c(COD_NOMBRE_REF:ESQ_END_REF,finDeSem,nEventosAll,ID_segmento)) %>%
      mutate(across(c(nEventosMedio:porcDias),round)) %>% select(-c(N4Sum,N34Sum,nEventosMedio_All,difN34))
    ...
    ####################################################
    ### Datos serie
    aa =  pruebaS %>% filter(ID_segmento == event$id)
    ...
    output$table <- renderFormattable({formattable(cc, list(`N34medio_All` = color_tile("white", "red"),
