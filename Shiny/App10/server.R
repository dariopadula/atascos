...
datSTM_Filtro = eventReactive(input$run, {
  ...
  IndFecha = length(diasIntercept) < nrow(dsma)
  if(IndFecha) {
    anioSelect = unique(dsma[diasIntercept,]$anio)
    mesSelect = as.numeric(unique(dsma[diasIntercept,]$mes))
    diaSelect = unique(dsma[diasIntercept,]$levelDia)
    
    datFiltro = subset(datFiltro,anio %in% anioSelect & 
                         mes %in% mesSelect &
                         levelDia %in% diaSelect)
  }
  ...
  datFiltro$posMin = datFiltro$minutoHora - minHoraIni
  datFiltro$posFin = minHoraFin - datFiltro$minutoHora - lInt
  # datFiltro$posFin = datFiltro$minutoHora - minHoraFin
  
  datFiltro = datFiltro %>% filter(posMin >= -(lInt-1) & posFin >= -(lInt-1)) %>%
    mutate(avg_pasadas_aj = ifelse(posMin < 0,((lInt + posMin)/lInt)*avg_pasadas,
                                ifelse(posFin < 0,((lInt + posFin)/lInt)*avg_pasadas,avg_pasadas)),
           subidas_aj = ifelse(posMin < 0,((lInt + posMin)/lInt)*avg_subidas,
                                    ifelse(posFin < 0,((lInt + posFin)/lInt)*avg_subidas,avg_subidas)),
           bajadas_aj = ifelse(posMin < 0,((lInt + posMin)/lInt)*avg_bajadas,
                               ifelse(posFin < 0,((lInt + posFin)/lInt)*avg_bajadas,avg_bajadas)))
...
#############################
##### Agrega datos STM
datSTM_Agg = eventReactive(input$run, {
  
  datFiltro = datSTM_Filtro()
  minHoraIni = intTiempo()[['minHoraIni']]
  minHoraFin = intTiempo()[['minHoraFin']]
  ...
  ####### Datos para agregar
  varsGroup = c('ID_parada')
  
  
  factorInterv = (minHoraFin - minHoraIni)/30
  
  datAgg =  datFiltro %>% group_by_at(vars(dplyr::one_of(varsGroup))) %>%
    summarise(count = n(),
              avg_pasadas_mean = round(mean(avg_pasadas_aj,na.rm = T)),
              avg_pasadas_max = round(max(avg_pasadas_aj,na.rm = T)),
              avg_pasadas_min = round(min(avg_pasadas_aj,na.rm = T)),
              avg_pasadas_sd = round(sd(avg_pasadas_aj,na.rm = T),1),
              avg_pasadas_intTime = avg_pasadas_mean*factorInterv) %>% 
    ungroup() %>%
    left_join(puntSTMUnic, by = 'ID_parada') %>%
        mutate(content = paste(sep = "<br/>",
                           paste("<b><a>Total reportes</a></b>:",count),
                           "<em><b><a>Volumen</a></em></b></font>:",
                           paste("<a>Prom 30 minutos</a>:",avg_pasadas_mean),
                           paste("<a>SD</a>:",avg_pasadas_sd),
                           paste("<a>Máximo 30 minutos</a>:",avg_pasadas_max),
                           paste("<a>Mínimo 30 minutos</a>:",avg_pasadas_min),
                           paste("<a>Prom Intevalo</a>:",avg_pasadas_intTime)),
               opacity = ifelse(avg_pasadas_intTime/max(avg_pasadas_intTime) < 0.1,0.1,avg_pasadas_intTime/max(avg_pasadas_intTime)),
               weight = ifelse(avg_pasadas_intTime/max(avg_pasadas_intTime) < 0.1,0.3,avg_pasadas_intTime/max(avg_pasadas_intTime)*5))
  ...
  # if(addCortes) {
    ...
    pp = pp %>% 
      ...
      addCircleMarkers(data = datSTM_Agg,
                       ...
                       radius = 10,
                       weight = ~weight,
                       color = ~palSTM(avg_pasadas_intTime),
                       fillColor = ~palSTM(avg_pasadas_intTime),
                       fillOpacity = ~opacity,
                       popup = ~content,
                       group = 'STM') %>%
      addLegend(position = 'topleft',pal = palSTM,values = datSTM_Agg$avg_pasadas_intTime,
                title = 'Omnibus promedio',group = 'STM')
      
    
    capasAdd = c(capasAdd,'STM','AutoScope','Corte')
